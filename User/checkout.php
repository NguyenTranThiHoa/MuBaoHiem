...
if (!isset($_SESSION['id'])) {
    header("Location: dangnhap.php?error=Bạn cần đăng nhập để thanh toán");
    exit();
}
?>
<!---------------------------Code y cũ-----------------------------------------------> 
...
//mysqli_query($conn,$sqli);
    ...
    // Xóa cart
    unset($_SESSION['cart']);
    header("Location: ChucMungDatHang.php");
}
?>
<!------------------------------------------------------------------------------------> 
<!DOCTYPE html>
        ...
        <div class="checkout_form">
            <div class="checkout-form-left">
                ...
                <form action="checkout.php" method="post">
                    <div class="row1">
                        <div class="col-lg-6">
                            <div class="checkout_input">
                                ...
                                <input type="text" placeholder="Nhập ghi chú" name="firstname" required>
                            </div>
                        </div>
                        <div class="col-lg-6">
                            <div class="checkout_input">
                                <p>Họ và tên<span>*</span></p>
                                <input type="text" placeholder="Nhập họ và tên" name="lastname" required>
                            </div>
                        </div>
                    </div>
                    <div class="checkout_input">
                        <p>Địa chỉ nhận hàng<span>*</span></p>
                        <input type="text" placeholder="Đại chỉ" class="checkout_input_add" name="address" required>
                    </div>
                    <div class="row1">
                        <div class="col-lg-6">
                            <div class="checkout_input">
                    ...
                    <div class="col-lg-6">
                            <input type="radio" name="gender" value="female">Thanh toán khi nhận hàng
                    </div>              
            </div>
            <div class="checkout-form-right">
                <div class="col-lg-4 col-md-6">
                    <div class="checkout_order">
                        <h4>Đơn hàng</h4>
                        <div class="checkout_order_products">Sản phẩm<span>Thành tiền</span>
                        </div>
                        <ul>
                            <?php
                            ...
                            ?>
                                <li><?=$item['name']?><img src="/QLbanhang1/admin/assets/uploads/sanpham/<?= $item['hinhanh'] ?>" style="max-width: 100px; margin-left:40px"><span style="margin-left:50px">
                                <?= $item['fav_language']?><span style="margin-left:30px"><?= $item['qty'] ?><span style="margin-left:30px"><?= number_format($item['price'] * $item['qty'], 0, '.', '') ?>.000VND
                                </span></li>
                            <?php }?>
                        </ul>
                        <div class="checkout_order_total"><b>Tổng tiền: </b><span style="margin-left:430px;">
                                <b><?= number_format($total, 0, '.', '').".000VND" ?></b>
                            </span></div>
                            <form action="checkout.php" method="post">
...
<?php if (isset($_SESSION['id'])) { ?>
    <?php } else { ?>
        <!-- Hiển thị thông báo và chuyển hướng đến trang đăng nhập nếu chưa đăng nhập -->
        <p>Vui lòng <a href="dangnhap.php">đăng nhập</a> để thanh toán.</p>
    <?php } ?>
</form>  
<!-----------------------------------------------------------> 
<footer>
